...
//            $this->logger->debug($response);
    ...
    public function getPaymentIntentID($token, $bodyRequest, $testEnv)
    {
        ...
        try {
            ...
            } elseif (str_contains($response, 'currency_not_allowed')) {
    ...
    /**
     * Fetch payment information from Getnet API.
     *
     * @param string $token
     ...
     * @return string
     */
    public function getPaymentInfo($token, $testEnv, $paymentId)
    {
        $this->logger->debug('----------------getPaymentInfo--------------------');

        $baseUrl = $testEnv === '1'
            ? 'https://api.pre.globalgetnet.com/digital-checkout/v1/payments/'
            : 'https://api.globalgetnet.com/digital-checkout/v1/payments/';

        $url = $baseUrl . urlencode($paymentId);
        ...
        try {
            ...
            $this->_curl->get($url);
            $response = $this->_curl->getBody();
            $this->logger->debug('Response service payment info --> ' . $response);
        } catch (\Magento\Framework\Exception\Exception $e) {
            $this->logger->debug($e);
            $this->messageManager->addError(__('Error al consultar la información del pago'));
            $response = '';
        }
